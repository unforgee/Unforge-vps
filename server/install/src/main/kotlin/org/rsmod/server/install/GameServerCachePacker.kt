...
class GameServerCachePacker : CliktCommand(name = "cache-pack") {
    ...
    private fun resolveAllTypes(injector: Injector): Boolean {
        ...
        // Ensure type lists are decoded, then apply item clones so `find()` references to clone
        // objs resolve and the clones get packed into the caches.
        injector.getInstance(TypeListMap::class.java)
        injector.getInstance(UnforgeCloneObjsLoader::class.java).applyClones()
