...
import org.rsmod.api.cache.types.enums.SpellbookSublistEnumPatch
...
import org.rsmod.content.other.commands.UnforgeCloneObjsLoader
...
class GameServer(private val skipTypeVerificationOverride: Boolean? = null) :
    ...
    fun prepareGame(injector: Injector) {
        // Ensure spellbook sublist key 0 exists before types/Js5Store load. Safe/idempotent when
        // already patched; required after TypeUpdaterCacheSync restores vanilla js5.
        SpellbookSublistEnumPatch.apply(gameCacheDir, js5CacheDir)
        loadCache(injector)
        loadMap(injector)
        ...
        loadCustomUiContent()
        loadSharedCustomContent()
    }

    private fun loadSharedCustomContent() {
        val root = DirectoryConstants.DATA_PATH.resolve("content")
        val registry = org.rsmod.server.app.content.Rsps2ContentRegistry(root)
        val records = registry.load()
        val errors = registry.validate(records)
        check(errors.isEmpty()) { "Invalid custom content: ${errors.joinToString("; ")}" }
        val counts =
            registry
                .byType(records)
                .entries
        ...
        logger.info { "Loaded ${records.size} shared r239 content records [$counts]" }
    }

    private fun loadCustomUiContent() {
        val root = DirectoryConstants.DATA_PATH.resolve("content")
        val definitions = org.rsmod.server.app.content.Rsps2UiContentRegistry(root).loadAll()
        logger.info { "Loaded ${definitions.size} custom r239 UI definitions from $root" }
    }
    ...
    private fun resolveAllTypes(injector: Injector) {
        ...
        // Inject item clones into the obj type list before references resolve, so `find()`
        // references to clone objs do not fail verification.
        loadItemClones(injector)
    ...
    private fun loadItemClones(injector: Injector) {
        val applied = injector.getInstance(UnforgeCloneObjsLoader::class.java).applyClones()
        if (applied.isNotEmpty()) {
            logger.info { "Applied ${applied.size} item clone type(s)." }
        }
    }
    ...
    private fun updateCacheConfigs(injector: Injector) {
        ...
        updater.updateAll()
        // syncFromBaseCaches copies vanilla → js5 and drops key-0 spellbook entries; re-apply.
        SpellbookSublistEnumPatch.apply(gameCacheDir, js5CacheDir)
    }
