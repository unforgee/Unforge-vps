...
constructor(
    ...
    override fun ScriptContext.startup() {
        onPlayerLogin { player.ensureDefaultCapacity() }

        onIfModalButton(bank_components.side_inventory) { sideInvOp(it.comsub, it.op) }
        onIfModalButton(bank_components.main_inventory) { mainInvOp(it.comsub, it.op, it.obj) }
    ...
    private suspend fun ProtectedAccess.mainInvOp(
        displayedSlot: Int,
        op: IfButtonOp,
        clickedObj: UnpackedObjType?,
    ) {
        val slot = resolveBankSlot(displayedSlot, clickedObj)
        logger.info {
            "[BANK-DEBUG] mainInvOp displayedSlot=$displayedSlot resolvedSlot=$slot op=$op " +
                "obj=${clickedObj?.id} player=${player.username}"
        }
    ...
    /**
     * The bank client can render a filtered or non-main tab using a display index that is not the
     * backing bank index. Prefer the index from the packet when it still contains the clicked
     * object; otherwise resolve the clicked object in the selected tab (and finally the whole
     * bank). This keeps ordinary slot clicks fast while preventing a valid click from becoming a
     * silent no-op after tab/search reordering.
     */
    private fun ProtectedAccess.resolveBankSlot(
        displayedSlot: Int,
        clickedObj: UnpackedObjType?,
    ): Int {
        if (clickedObj == null) return displayedSlot

        val displayed = bank.takeIf { displayedSlot in it.indices }?.get(displayedSlot)
        if (displayed != null && objTypes[displayed].id == clickedObj.id) {
            return displayedSlot
        }

        val selectedMatch =
            selectedTab.slotRange(this).firstOrNull { slot ->
                bank[slot]?.let { objTypes[it].id == clickedObj.id } == true
            }
        if (selectedMatch != null) return selectedMatch

        return bank.indices.firstOrNull { slot ->
            bank[slot]?.let { objTypes[it].id == clickedObj.id } == true
        } ?: displayedSlot
    }

    private suspend fun ProtectedAccess.mainInvWithdraw(slot: Int, op: IfButtonOp) {
        player.ensureDefaultCapacity()
        val obj =
            bank[slot]
                ?: run {
                    logger.info {
                        "[BANK-DEBUG] withdraw: bank[$slot] is null player=${player.username}"
                    }
                    return resendSlot(bank, 0)
                }
        ...
        logger.info {
            "[BANK-DEBUG] withdraw: slot=$slot op=$op obj=$obj count=$count result=$result " +
                "cert=$withdrawCert placehold=${alwaysPlacehold || op == IfButtonOp.Op9}"
        }
    ...
    private suspend fun ProtectedAccess.sideInvOp(slot: Int, op: IfButtonOp) {
        logger.info { "[BANK-DEBUG] sideInvOp slot=$slot op=$op player=${player.username}" }
        if (op == IfButtonOp.Op10) {
            objExamine(inv, slot)
            return
        }
    ...
    private fun ProtectedAccess.invDeposit(slot: Int, count: Int, inventory: Inventory): Boolean {
        ...
        logger.info {
            "[BANK-DEBUG] deposit: slot=$slot count=$count obj=$obj intoSlot=${prioritySlot ?: tabSlots.first} " +
                "result=$result results=${insertQuery.results} bankOccupied=${bank.occupiedSpace()} " +
    ...
    private fun ProtectedAccess.selectTab(comsub: Int, op: IfButtonOp) {
        ...
        when (op) {
            ...
            else -> logger.debug { "Ignoring unhandled bank tab op: comsub=$comsub op=$op" }
        }
    }
    ...
    private fun Player.ensureDefaultCapacity() {
