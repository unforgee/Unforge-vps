...
/**
 * Toggles walk/run from the minimap orb and the settings-tab run button.
 *
 * Both call the same [org.rsmod.api.player.protect.ProtectedAccess.toggleRun] path so `option_run`,
 * move speed, and the orb graphic stay in sync. The orb click target must be the component with the
 * "Toggle Run" op (`orbs:28`), not the decorative `orbs:runbutton` graphic.
 */
class RunModeScript
@Inject
