...
constructor(
    ...
    private fun ProtectedAccess.cut(tree: BoundLocInfo, type: UnpackedLocType) {
        ...
        if (cutLogs) {
            ...
            skillingRewards.grant(this, "WOODCUTTING", product)
            // Lumberjack perk: chance of a bonus log.
            if (random.of(10_000) < perks.doubleLogsBps(player)) {
                invAdd(inv, product)
            }
    ...
    companion object {
        ...
        fun findAxe(player: Player, objTypes: ObjTypeList): InvObj? {
            return (player.inv.filterNotNull { objTypes[it].isUsableAxe(player.woodcuttingLvl) } +
                    listOfNotNull(player.wornAxe(objTypes)))
                .maxWithOrNull(
                    compareBy<InvObj> { objTypes[it].axeWoodcuttingReq }
                        .thenBy { if (it.id == objs.bronze_axe.id) 0 else 1 }
                )
        }
