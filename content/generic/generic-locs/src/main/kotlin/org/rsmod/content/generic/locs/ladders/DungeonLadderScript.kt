...
class DungeonLadderScript : PluginScript() {
    override fun ScriptContext.startup() {
        onOpLoc1(content.dungeonladder_down) { climbDown(it.loc, it.type) }
        onOpLoc1(content.dungeonladder_up) { climbUp(it.loc, it.type) }
    }

    private suspend fun ProtectedAccess.climbUp(
        loc: BoundLocInfo,
        type: UnpackedLocType,
    ): Unit = climb(loc, type, -6400)
    ...
    private suspend fun ProtectedAccess.climb(
        ...
        arriveDelay()
        val dest = loc.coords.translateZ(translateZ)
        anim(type.climbAnim())
        delay(1)
        telejump(dest)
    }
