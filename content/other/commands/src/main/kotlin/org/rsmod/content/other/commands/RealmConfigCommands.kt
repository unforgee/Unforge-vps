...
constructor(
    ...
    override fun ScriptContext.startup() {
        ...
        onCommand("xprate", "Set the global XP rate multiplier", ::xpRate) {
            invalidArgs = "Use as ::xprate rate (e.g. ::xprate 50 or ::xprate 150x)"
        }
    }

    private fun xpRate(cheat: Cheat) =
        with(cheat) {
            val rate = args[0].trimEnd('x', 'X').toDouble()
            if (rate <= 0.0 || rate > 100_000.0) {
                player.mes("Invalid XP rate. Use as ::xprate rate (e.g. ::xprate 50)")
                return
            }
            applyChanges(player, realm.config.copy(globalXpRate = rate))
            val text = if (rate % 1.0 == 0.0) "${rate.toInt()}" else "$rate"
            player.mes("<col=006600>Global XP rate set to x$text for all players.</col>")
        }
