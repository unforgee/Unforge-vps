...
public class Player(
    ...
    /**
     * The map clock at which the player's active teleblock expires, or `-1` when the player is not
     * teleblocked.
     *
     * While teleblocked, the player is prevented from casting teleport spells.
     */
    public var teleblockExpiry: Int = -1

    /** The damage dealt per tick by the player's active poison, or `0` when not poisoned. */
    public var poisonDamage: Int = 0

    /** The map clock at which the player's active poison expires, or `-1` when not poisoned. */
    public var poisonExpiry: Int = -1
    ...
    public var members: Boolean = true
    ...
    /**
     * Whether [modLevel] has been assigned. Live logins assign it during account load before
     * `SessionStateEvent.Login`, but test-harness players may remain unassigned - rank-icon code
     * paths must probe this instead of dereferencing [modLevel] blindly.
     */
    public val isModLevelAssigned: Boolean
        get() = ::modLevel.isInitialized

    public var xpRate: Double = 1.0
    public var globalXpRate: Double = 1.0

    /*
     * Server-authoritative game mode (ironman) state. `gameMode == null` together with
     * `gameModeSelected == false` marks an account that has not yet chosen a mode; such
     * accounts are restricted from normal gameplay by [org.rsmod.game.ironman.IronmanPolicy]
     * until the selection has been persisted. Normal commands or client input must never
     * modify these fields; changes go through the ironman selection/admin services only.
     */
    public var gameMode: GameMode? = null
    public var gameModeSelected: Boolean = false
    public var gameModeSelectedAt: LocalDateTime? = null

    /*
     * Server-authoritative difficulty tier. `difficulty == null` together with
     * `difficultySelected == false` marks an account that has not yet picked a tier; the
     * first-login selection modal re-opens until the choice is persisted. Selection writes
     * [Difficulty.xpRate] into [xpRate] (persisted via `xp_rate_in_hundreds`), and the
     * tier's damage/drop boosts apply through the hit-modify and drop-roll seams.
     */
    public var difficulty: Difficulty? = null
    public var difficultySelected: Boolean = false
    public var difficultySelectedAt: LocalDateTime? = null
    public var hardcoreStatus: HardcoreStatus = HardcoreStatus.DISABLED
    public var hardcoreDeathCount: Int = 0
    public var groupId: Int? = null
    public var groupRank: GroupRank? = null
    public var groupJoinedAt: LocalDateTime? = null
    public var groupSettingsVersion: Int = 0

    /** Convenience: the hardcore life is still intact. */
    public val hardcoreRemaining: Boolean
        get() = gameMode?.isHardcore == true && hardcoreStatus.hasHardcoreRemaining

    public var publicMessage: PublicMessage? = null

    /**
     * Index into the client's mod-icons array (`<img=N>` tags / chat crown byte) of this player's
     * secondary badge, or `-1` when none is set. The badge is shown alongside the staff rank icon
     * resolved from [modLevel] - e.g. a donator tier badge - and is never consulted for staff
     * authorization, which remains `modLevel`-only.
     */
    public var chatBadge: Int = -1
