...
public sealed class PathingEntity {
    ...
    /**
     * The map clock at which this entity's movement lock expires, or `-1` when movement is not
     * locked.
     *
     * A movement lock prevents the entity from starting _or_ continuing movement and is used for
     * effects such as freeze and bind. The movement processors enforce the lock, and it expires
     * automatically once the map clock passes [movementLockExpiry].
     *
     * @see [lockMovement]
     * @see [isMovementLocked]
     */
    public var movementLockExpiry: Int = -1
        private set

    /** `true` if this entity's movement is currently locked. */
    public val isMovementLocked: Boolean
        get() = movementLockExpiry > currentMapClock
    ...
    /**
     * Locks this entity's movement for [cycles], preventing it from starting or continuing
     * movement. An existing lock is only refreshed when [cycles] would expire later than it already
     * does.
     *
     * @see [isMovementLocked]
     * @see [clearMovementLock]
     */
    public fun lockMovement(cycles: Int) {
        require(cycles > 0) { "`cycles` must be greater than 0. (cycles=$cycles)" }
        val expiry = currentMapClock + cycles
        if (expiry > movementLockExpiry) {
            movementLockExpiry = expiry
        }
    }

    /** Removes any active movement lock, allowing this entity to move again immediately. */
    public fun clearMovementLock() {
        movementLockExpiry = -1
    }
