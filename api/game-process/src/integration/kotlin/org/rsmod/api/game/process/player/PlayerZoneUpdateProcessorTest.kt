...
class PlayerZoneUpdateProcessorTest {
    ...
    fun GameTestState.`process zones in a new build area`() = runAdvancedGameTest {
        ...
        withPlayer(startCoords) {
            ...
            // `sendZonePersistentUpdates`
            //
            // The spawned loc and the private obj are rebuilt into one observer-specific
            // `UpdateZonePartialEnclosed` for the start zone, not written as standalone zone
            // prots - so the payload is where their contents have to be verified.
            assertEquals(0, captured.count { it is ZoneProt })
            assertEquals(1, captured.countOf<UpdateZonePartialEnclosed>())
            assertEnclosedZone(
                message = captured.single<UpdateZonePartialEnclosed>(),
                zoneBase = startZone.toCoords(),
                buildArea = buildArea,
                // Only the spawned loc: the static map loc must not make it into the payload.
                LocAddChangeV2(standardLoc, 2, 2, 10, 0, OpFlags.ALL_SHOWN),
                ObjAdd(standardObj, 1, 1, 2, OpFlags.ALL_SHOWN),
            )
    ...
    fun GameTestState.`only send private obj to receiver`() = runAdvancedGameTest {
        ...
        val startCoords = CoordGrid(0, 1, 1, 32, 32)
        val objZoneBase = ZoneKey.from(startCoords).toCoords()
        ...
        // Obj should be sent to `client2`, enclosed in an observer-specific zone update.
        assertEquals(0, client2.count { it is ZoneProt })
        assertEquals(1, client2.countOf<UpdateZonePartialEnclosed>())
        assertEnclosedZone(
            message = client2.single<UpdateZonePartialEnclosed>(),
            zoneBase = objZoneBase,
            buildArea = player2.buildArea,
            ObjAdd(stackableObj, 1, 1, 0, OpFlags.ALL_SHOWN),
        )
    ...
    /**
     * Asserts that [message] targets the zone at [zoneBase] within [buildArea], and that its
     * payload holds exactly [prots], in order.
     *
     * The payload is opaque once encoded, so the expectation is built by running [prots] through
     * the same [ZonePartialEnclosedCacheBuffer] the processor uses and comparing the bytes. That
     * pins down the prot types and every field the encoder writes; what it does not - and cannot -
     * check is whether rsprot's own encoders are correct.
     */
    private fun assertEnclosedZone(
        message: UpdateZonePartialEnclosed,
        zoneBase: CoordGrid,
        buildArea: CoordGrid,
        vararg prots: ZoneProt,
    ) {
        assertEquals(zoneBase.x - buildArea.x, message.zoneX)
        assertEquals(zoneBase.z - buildArea.z, message.zoneZ)
        assertEquals(zoneBase.level, message.level)

        val cache = ZonePartialEnclosedCacheBuffer(listOf(OldSchoolClientType.DESKTOP))
        try {
            val expected = cache.computeZoneForClient(OldSchoolClientType.DESKTOP, prots.asList())
