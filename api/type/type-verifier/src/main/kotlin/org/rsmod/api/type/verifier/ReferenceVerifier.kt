    ...
    public fun verifyAll(verifyIdentityHashes: Boolean): Verification {
        val updates =
            if (!verifyIdentityHashes) {
                // On revision bumps, missing vanilla ids are symbol drift — not something the
                // packer can invent. Ignore them when identity-hash verification is skipped.
                references.updates.filter { it.status !is TypeReferenceResult.CacheTypeNotFound }
            } else {
                references.updates
            }
