    ...
    public fun decode(builder: LocTypeBuilder, data: ByteBuf, code: Int): Unit =
        with(builder) {
            when (code) {
                ...
                // Newer ambient-sound / raise payloads (consume; fields not yet modeled).
                91 -> data.readUnsignedByte()
                93 -> {
                ...
                95 -> data.readUnsignedByte()
                96 -> data.readUnsignedByte()
                // Entity op extensions (consume; not yet modeled for server use).
                100 -> {
                    data.readUnsignedByte()
                    data.readUnsignedByte()
                    data.readString()
