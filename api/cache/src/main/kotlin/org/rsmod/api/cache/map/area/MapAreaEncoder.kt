...
public object MapAreaEncoder {
    ...
    public fun encodeAll(
        ...
        // Map areas are a server-only file within the mapsquare group.
        if (ctx.clientOnly) {
            return
        }
        ...
        for ((key, area) in areas) {
            ...
            val oldBuf = cache.readOrNull(archive, key.id, MapGroupFiles.AREAS)
            val newBuf = buffer.clear().apply { encode(area, this) }
            if (newBuf != oldBuf) {
                cache.write(archive, key.id, MapGroupFiles.AREAS, newBuf)
            }
            oldBuf?.release()
        }
        buffer.release()
