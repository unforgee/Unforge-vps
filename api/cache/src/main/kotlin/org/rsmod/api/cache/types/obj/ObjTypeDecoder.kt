    ...
    public fun decode(builder: ObjTypeBuilder, data: ByteBuf, code: Int): Unit =
        with(builder) {
            when (code) {
                ...
                9 -> data.readString() // unknown1 client string
                11 -> stackable = true
                ...
                15 -> tradeable = false
                16 -> {
                    // Opcode 16 is a legacy gated-item flag. Consume it for compatibility but
                    // deliberately leave the flag disabled in Unforge.
                }
                23 -> {
                ...
                // Int model ids (models past ushort range).
                ...
                160 -> {
                    /* stackable = 2 (always-stack client flag); treat as stackable */
                    stackable = true
                }
                200 -> {
                    val count = data.readUnsignedByte().toInt()
                ...
                212 -> weaponCategory = data.readUnsignedByte().toInt()
