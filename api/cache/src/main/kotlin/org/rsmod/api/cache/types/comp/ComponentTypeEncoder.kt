...
public object ComponentTypeEncoder {
    /**
     * Encodes [types] into the interfaces archive, where each component is stored as file `id and
     * 0xFFFF` within group `id ushr 16` (the parent interface id).
     *
     * Unlike config-archive encoders, component data is positional rather than opcode-based, so no
     * terminator byte is appended and there is no server-only tail - the same bytes are written to
     * both the game and js5 caches.
     */
    ...
    public fun encodeV1(unpacked: UnpackedComponentType, data: ByteBuf): Unit =
        with(unpacked) {
            ...
            if (type == 6) {
                // Models are stored as ints on this revision - the decoder reads them back with
                // `readInt`. (`writeShort` here would corrupt any round trip.)
                data.writeInt(model)
    ...
    public fun encodeV3(unpacked: UnpackedComponentType, data: ByteBuf): Unit =
        with(unpacked) {
            ...
            if (type == 6) {
                // Stored as an int on this revision - see the note in [encodeV1].
                data.writeInt(model)
