...
public object ObjTypeEncoder {
    ...
    public fun encodeJs5(type: UnpackedObjType, data: ByteBuf, ctx: EncoderContext): Unit =
        with(type) {
            // Rev 239+ uses 4-byte model opcodes (44–54). Legacy short opcodes (1/23–26/90–93)
            // leave clients unable to finish decoding, so params like spell_button (596) are lost.
            if (model != 0) {
                data.writeByte(44)
                data.writeInt(model)
            }
            ...
            // Unforge does not have gated items. Never emit the legacy opcode 16; otherwise a
            // client can rewrite the item name/examine text or hide actions.
