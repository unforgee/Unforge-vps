    ...
    public fun encodeJs5(type: UnpackedNpcType, data: ByteBuf, ctx: EncoderContext): Unit =
        with(type) {
            if (models.isNotEmpty()) {
                // Revision 239 npc defs use 32-bit model ids (opcode 61). The client only
                // decodes opcode 61 for models; opcode 1 (16-bit) is an unhandled no-op that
                // desyncs its decoder and crashes the npc type loader.
                data.writeByte(61)
            ...
            if (head.isNotEmpty()) {
                // Revision 239 uses 32-bit chathead model ids (opcode 62); see opcode 61 note.
                data.writeByte(62)
            ...
            if (hitpoints != NpcTypeBuilder.DEFAULT_HITPOINTS) {
                data.writeByte(77)
