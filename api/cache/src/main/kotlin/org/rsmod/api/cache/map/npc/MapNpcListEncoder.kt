...
public object MapNpcListEncoder {
    public fun encodeAll(
        ...
        // Map npc spawns are a server-only file within the mapsquare group.
        if (ctx.clientOnly) {
            return
        }
        ...
        for ((key, definition) in spawns) {
            val oldBuf = cache.readOrNull(archive, key.id, MapGroupFiles.NPCS)
            val newBuf = buffer.clear().apply { encode(definition, this) }
            if (newBuf != oldBuf) {
                cache.write(archive, key.id, MapGroupFiles.NPCS, newBuf)
            }
            oldBuf?.release()
        }
        buffer.release()
