    ...
    public fun decode(builder: NpcTypeBuilder, data: ByteBuf, code: Int): Unit =
        with(builder) {
            when (code) {
                ...
                // Newer client-only npc flags / footprint / overlap (consume if unmodeled).
                126 -> data.readUnsignedShort()
                129 -> {
                    /* unknown1 */
                }
                130 -> {
                ...
                146 -> data.readUnsignedShort()
                147 -> {
                    /* zbuf = false */
                }
                // Client entity-op extensions (consume; not yet modeled).
                251 -> {
