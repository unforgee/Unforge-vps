    ...
    public fun encodeJs5(type: UnpackedLocType, data: ByteBuf, ctx: EncoderContext): Unit =
        with(type) {
            if (models.isNotEmpty() && shapes.isNotEmpty()) {
                // Revision 239 loc defs use 32-bit model ids (opcode 6). The client only
                // decodes opcodes 6/7 for models; opcode 1/5 (16-bit) desyncs its decoder.
                data.writeByte(6)
            ...
            if (models.isNotEmpty() && shapes.isEmpty()) {
                // Revision 239 uses 32-bit model ids (opcode 7); see opcode 6 note above.
                data.writeByte(7)
                data.writeByte(models.size)
                for (model in models) {
                    data.writeInt(model)
