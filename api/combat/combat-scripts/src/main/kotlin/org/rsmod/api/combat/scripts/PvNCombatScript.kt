...
import org.rsmod.api.script.advanced.onDefaultApNpc1
import org.rsmod.api.script.advanced.onDefaultApNpc2
import org.rsmod.api.script.advanced.onDefaultOpNpc1
...
constructor(
    ...
    override fun ScriptContext.startup() {
        onDefaultApNpc1 {
            if (it.npc.visType.op.getOrNull(0)?.equals("attack", ignoreCase = true) == true) {
                attemptCombatAp(it.npc)
            } else {
                apRange(-1)
            }
        }
    ...
    private fun ProtectedAccess.canAttack(npc: Npc): Boolean {
        if (player.coords.isWildernessSafeZone() != npc.coords.isWildernessSafeZone()) {
            mes("Combat is forbidden across sanctuary boundaries.")
            clearPendingAction()
            return false
        }
        ...
        if (singleCombat) {
            ...
            if (npc.lastCombat + ACTIVE_COMBAT_DELAY > mapClock) {
                if (npc.aggressivePlayer != null && npc.aggressivePlayer != player.uid) {
                    val aggPlayer = npc.aggressivePlayer?.resolve(playerList)
                    if (
                        aggPlayer == null || !companionPlayerRegistry.areAllied(player, aggPlayer)
                    ) {
