...
constructor(
    ...
    public fun attemptCast(
        ...
        if (player.username.startsWith("companion_")) {
    ...
    public fun canCastSpell(
        ...
        if (player.username.startsWith("companion_")) return true
        val accessFailure = validateAccess(player, spell, book)
        if (accessFailure != null) {
            return false
        }
    ...
    private fun delAll(player: Player, validations: List<MagicRunes.Validation.Valid>): CastResult {
        ...
        // Worn skilling gear can preserve the runes the spell just consumed.
        if (
            consume.isNotEmpty() &&
                wornSkills.value(player, "MAGIC", "MAGIC_RUNE_SAVE_CHANCE") >
                    kotlin.random.Random.nextInt(10_000)
        ) {
            for (obj in consume) {
                player.invAdd(player.inv, obj.id, obj.count, vars = obj.vars, strict = false)
            }
            player.mes("<col=6699ff>Your gear preserves the runes for that spell.</col>")
        }
