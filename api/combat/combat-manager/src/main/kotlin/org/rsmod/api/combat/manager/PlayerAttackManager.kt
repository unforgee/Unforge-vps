...
import org.rsmod.api.companion.CompanionHitmarkResolver
...
import org.rsmod.api.npc.hit.visHitmark
...
constructor(
    ...
    public fun continueCombat(source: Player, target: Npc) {
        npcInteractions.interact(source, target, target.attackOp())
    }
    ...
    private fun rollMeleeAccuracy(
        ...
        accuracy.rollMeleeAccuracy(
            ...
            attackStyle = attackStyle,
            blockType = blockType,
            // `Battle Hymn` accuracy rides the multiplier the formulae already apply to the hit
            // chance. Only this npc-targeted overload does it, so PvP is unaffected.
            specMultiplier = specMultiplier * supportAccuracyMultiplier(source, target),
            random = random,
        )
        ...
        when (target) {
            is Npc ->
                queueMeleeHit(
                    source,
                    target,
                    companionAdjustedDamage(source, damage),
                    delay,
                    hitmark,
                )
            is Player ->
                queueMeleeHit(source, target, companionAdjustedDamage(source, damage), delay)
        }
    ...
    private fun queueMeleeHit(
        ...
        val hit =
            target.queueHit(
                ...
                damage,
                npcHitModifier,
                hitmark = hitmark ?: companionHitmark(source) ?: target.visHitmark(),
            )
    ...
    private fun rollSpellAccuracy(
        ...
        // The spell roll has no multiplier parameter, so the support accuracy bonus is applied to
        // the hit chance directly. Still npc-only, so PvP is unaffected.
        val chance =
        ...
        val bps =
            SupportNpcRules.scale(SupportCombatState.outgoingAccuracyBps(source), target.visType)
        val boosted =
            if (bps <= 0) chance
            else (chance.toLong() * (10_000L + bps) / 10_000L).coerceAtMost(10_000L).toInt()
        return AccuracyFormulae.isSuccessfulHit(boosted, random)
    }

    /**
     * `Battle Hymn` accuracy as a multiplier on the hit chance, scoped by the target npc's support
     * rules. Returns `1.0` when the buff is inactive so the roll is byte-for-byte unchanged.
     */
    private fun supportAccuracyMultiplier(source: Player, target: Npc): Double {
        val bps =
            SupportNpcRules.scale(SupportCombatState.outgoingAccuracyBps(source), target.visType)
        return if (bps <= 0) 1.0 else 1.0 + bps / 10_000.0
    }
        ...
        when (target) {
            is Npc ->
                queueMagicHit(
                    ...
                    target = target,
                    spell = spell,
                    damage = companionAdjustedDamage(source, damage),
    ...
    private fun companionAdjustedDamage(source: Player, damage: Int): Int {
        val multiplier = companionPlayerRegistry.getDamageMultiplier(source)
        return (damage * multiplier).toInt().coerceAtLeast(0)
    }

    private fun companionHitmark(source: Player): HitmarkTypeGroup? {
        val companionId = companionPlayerRegistry.getCompanionId(source) ?: return null
        ...
        return CompanionHitmarkResolver.group(
            companions.consumePendingAbilityHitmark(companionId)
                ?: CompanionHitmarkResolver.styleForCompanion(companion)
        )
    }
}
