...
public fun Npc.combatDefaultRetaliateOp(interactions: AiPlayerInteractions) {
    ...
    retaliate(target, interactions, ap = combatAttackStyle() != NpcAttackStyle.Melee)
}
...
/**
 * The combat attack style this npc uses, derived from the `npc_attack_style` param on its current
 * visual type. Defaults to [NpcAttackStyle.Melee] when the param is absent.
 */
public fun Npc.combatAttackStyle(): NpcAttackStyle =
    NpcAttackStyle.from(visType.param(params.npc_attack_style))
