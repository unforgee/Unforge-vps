...
public object UpdateInventory {
    /** @see [UpdateInvFull] */
    public fun updateInvFull(
        player: Player,
        inv: Inventory,
        equipmentInstances: EquipmentInstanceRegistry? = null,
        objTypes: ObjTypeList? = null,
    ) {
        ...
        if (equipmentInstances != null && objTypes != null) {
            EquipmentInstanceHoverSync.sync(player, inv, equipmentInstances, objTypes)
        }
    }

    /** @see [UpdateInvPartial] */
    public fun updateInvPartial(
        player: Player,
        inv: Inventory,
        equipmentInstances: EquipmentInstanceRegistry? = null,
        objTypes: ObjTypeList? = null,
    ) {
        val changedSlots = inv.modifiedSlots.asSequence().toList()
        val provider = RspIndexedObjProvider(inv.objs, changedSlots.iterator())
        val message = UpdateInvPartial(-1, -(1234 + inv.type.id), inv.type.id, provider)
        player.client.write(message)
        if (equipmentInstances != null && objTypes != null) {
            EquipmentInstanceHoverSync.sync(player, inv, equipmentInstances, objTypes, changedSlots)
        }
    }
