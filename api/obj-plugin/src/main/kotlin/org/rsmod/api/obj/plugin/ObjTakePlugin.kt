...
public class ObjTakePlugin @Inject constructor(private val repo: ObjRepository) : PluginScript() {
    ...
    private suspend fun ProtectedAccess.triggerTake(obj: Obj) {
        ...
        val takeCheck = IronmanPolicy.canTakeObj(player, obj)
        if (!takeCheck.allowed) {
            takeCheck.denialMessage?.let(player::mes)
            return
        }
        player.resetAnim()
    ...
    private fun Player.takeClose(obj: Obj) {
        // Re-check ownership at the point of the destructive repo op: the obj could have
        // changed (or the approach could have been initiated by another code path) since the
        // pre-walk check.
        val takeCheck = IronmanPolicy.canTakeObj(this, obj)
        if (!takeCheck.allowed) {
            takeCheck.denialMessage?.let(::mes)
            return
        }
    ...
    private fun Player.transaction(obj: Obj): TransactionResultList<InvObj> =
        invAdd(inv, obj.type, obj.count, instanceId = obj.instanceId, autoCommit = false)
}
