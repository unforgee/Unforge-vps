...
constructor(private val objTypes: ObjTypeList, private val enumResolver: EnumTypeMapResolver) {
    ...
    private lateinit var autocastIds: Map<Int, Int>
    ...
    /**
     * Returns the autocast registry id for [spell] (as stored in the `autocast_spell` varbit), or
     * `null` when the spell cannot be autocast.
     */
    public fun getAutocastId(spell: MagicSpell): Int? = autocastIds[spell.obj.id]

    /** All spells that can be selected for autocasting, keyed by their autocast registry id. */
    ...
    internal fun init() {
        ...
        this.autocastIds = autocastSpells.entries.associate { (id, spell) -> spell.obj.id to id }
    }
    ...
    private fun loadObjSpells(): Map<Int, MagicSpell> {
        ...
        for (spellbookEnum in spellbookList.values) {
            ...
            for (spellObj in spellList.values) {
                val spell = spellObj.toMagicSpell() ?: continue
    ...
    private fun ObjType.toMagicSpell(): MagicSpell? {
        ...
        // New revision spells may land in enums before cast-xp editors exist; skip until
        // configured.
