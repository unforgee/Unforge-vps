...
class InvTransactionsTest {
    ...
    @Test
    fun GameTestState.`equipment instance survives inventory and worn swaps`() = runBasicGameTest {
        withPlayerInit {
            val instanceId = 9_876_543_210L
            val invSlot = 5
            val wornSlot = Wearpos.RightHand.slot
            inv[invSlot] = InvObj(type = objs.abyssal_whip, count = 1, instanceId = instanceId)
            val equip =
                invSwap(from = inv, fromSlot = invSlot, intoSlot = wornSlot, into = worn).single()
            assertTrueContract(equip.isOk())
            assertEquals(instanceId, worn[wornSlot]?.instanceId)

            val unequip =
                invTransfer(from = worn, fromSlot = wornSlot, count = 1, into = inv).single()
            assertTrueContract(unequip.isOk())
            assertEquals(
                instanceId,
                inv.filterNotNull { it.instanceId == instanceId }.single().instanceId,
            )
        }
    }

    @Test
    fun GameTestState.`equipment instances never merge in bank`() = runBasicGameTest {
        withPlayerInit {
            val bank = invMap.getOrPut(cacheTypes.invs[invs.bank])
            inv[0] = InvObj(objs.abyssal_whip, instanceId = 101L)
            inv[1] = InvObj(objs.abyssal_whip, instanceId = 202L)
            assertTrueContract(invTransfer(inv, 0, 1, bank).single().isOk())
            assertTrueContract(invTransfer(inv, 1, 1, bank).single().isOk())
            assertEquals(2, bank.occupiedSpace())
            assertEquals(
                setOf(101L, 202L),
                bank.filterNotNull { it.instanceId != 0L }.map { it.instanceId }.toSet(),
            )
        }
    }
