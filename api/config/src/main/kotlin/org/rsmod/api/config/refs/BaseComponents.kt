...
object BaseComponents : ComponentReferences() {
    ...
    val chatbox_chatmodal = find("chatbox:chatmodal", 2917314310066654473)

    /**
     * The whole chat-text region, `519` wide by the chatbox height less its `23`-pixel tab strip.
     * Interfaces that replace the entire chat area rather than sitting in a dialogue box - the
     * skilling make-menu, for one - target this instead of the much smaller [chatbox_chatmodal].
     */
    ...
    // Do not use these to set the combat tab header text - clientscript 7592 owns both lines and
    // will fight any server-side write. The names also mislead: `:title` is the *layer* holding the
    // two text lines (so `ifSetText` on it does nothing), and `:category` is the top line, which
    // shows the weapon name.
    val combat_tab_title = find("combat_interface:title", 6291251021612374960)
    val combat_tab_category = find("combat_interface:category", 584437891524701911)

    val fade_overlay_message = find("fade_overlay:message", 9045250929562636923)

    val music_now_playing_text = find("music:now_playing_text", 4443528042368292600)
}
