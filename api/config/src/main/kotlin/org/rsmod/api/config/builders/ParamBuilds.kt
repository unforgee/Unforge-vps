...
internal object ParamBuilds : ParamBuilder() {
    init {
        ...
        // Npc combat attack style. 0 = melee, 1 = ranged, 2 = magic.
        build<Int>("npc_attack_style") { default = 0 }
        ...
        // Per-npc PvM support rules. Multipliers are per-mille (1000 = 100%); set 0 to block the
        // effect entirely for that npc. Defaults mean "full support", so nothing changes for the
        // existing npc roster until a boss opts out.
        build<Int>("npc_support_multiplier") { default = 1000 }
        build<Int>("npc_support_healing_multiplier") { default = 1000 }
        build<Int>("npc_support_special_multiplier") { default = 1000 }
        build<Int>("npc_pvm_tier") { default = 0 }
