...
object BaseInterfaces : InterfaceReferences() {
    ...
    val bank_main = find("bankmain", 9223372035766306703)
    val bank_side = find("bankside", 9223372036689025022)
    val bankpin_settings = find("bankpin_settings", 642937557)

    val toplevel = find("toplevel", 9223372035849431145)
    val toplevel_osrs_stretch = find("toplevel_osrs_stretch", 9223372036334532075)
    val toplevel_pre_eoc = find("toplevel_pre_eoc", 1832941617)
    ...
    val wornitems = find("wornitems", 9223372035153201241)
    val side_channels = find("side_channels", 1832628328)
    val settings_side = find("settings_side", 9223372036583730351)
    val prayerbook = find("prayerbook", 9223372034905333594)
    val magic_spellbook = find("magic_spellbook", 1700349896)
    val friends = find("friends", 149140580)
    val account = find("account", 9223372035358871987)
    val logout = find("logout", 2003439444)
    val emote = find("emote", 1866350332)
    val music = find("music", 9223372035978928327)
    val chatchannel_current = find("chatchannel_current", 555490233)
    val worldswitcher = find("worldswitcher", 770239458)
    val combat_interface = find("combat_interface", 9223372035840528241)
    ...
    /** The generic skilling "what would you like to make?" dialog. */
    ...
    val collection = find("collection", 9223372034947885929)
    val bond_main = find("bond_main", 9223372036023293853)
    val poh_options = find("poh_options", 2121095449)
