...
public object ZoneUpdateTransformer {
    ...
    /**
     * A zone prot whose visibility depends on who is observing the zone, so it cannot go in the
     * buffer shared between every observer of that zone. `SharedZoneEnclosedBuffers` filters these
     * out, and `PlayerZoneUpdateProcessor` encodes them per player instead.
     *
     * [backing] is the payload actually put on the wire; this wrapper only carries the extra state
     * needed to decide whether a given observer should see it.
     *
     * #### Naming
     * This was `PartialFollowsZoneProt`, from when these were written as standalone messages after
     * an `UpdateZonePartialFollows` header. They no longer are, and could not be: rsprot's
     * `Session.validateMessage` rejects every payload these wrap unless it is enclosed.
     */
    public interface PerObserverZoneProt : ZoneProt {
    ...
    public interface ObjPrivateZoneProt : PerObserverZoneProt {
        public val obj: Obj
