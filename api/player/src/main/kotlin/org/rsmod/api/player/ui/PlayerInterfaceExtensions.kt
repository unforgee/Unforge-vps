...
// TEMP protocol-audit logger — remove after rev-239 interface verification.
private val protocolAuditLogger = InlineLogger()
...
/** Sets the vertical scroll position (in pixels) of a scrollable layer [target]. */
public fun Player.ifSetScrollPos(target: ComponentType, scrollPos: Int) {
    client.write(IfSetScrollPos(target.interfaceId, target.component, scrollPos))
}
...
private fun Player.openOverlay(interf: InterfaceType, target: ComponentType, eventBus: EventBus) {
    ...
    // aware, the interface is being opened on the "base" target component. (when applicable)
    val translated = ui.translate(idComponent)
    // TEMP protocol-audit: remove after rev-239 interface verification.
    if (interf.id == 218 || interf.id == 116) {
        protocolAuditLogger.info {
            "[protocol-audit] IfOpenSub overlay iface=${interf.id} " +
                "base=${idComponent.parent}:${idComponent.child} " +
                "translated=${translated.parent}:${translated.child} " +
                "type=${IfSubType.Overlay.id}"
        }
    }
...
internal fun Player.ifMesbox(text: String, pauseText: String, lineHeight: Int, eventBus: EventBus) {
    ...
    ifSetTextAlign(
        this,
        components.messagebox_text,
        alignH = 1,
        alignV = DIALOGUE_TEXT_ALIGN_V,
        lineHeight,
    )
...
/**
 * Opens interface 270 and populates it with [options].
 *
 * [optionComponents] must be `270:15` onwards, one per entry in [options], already resolved from
 * the component type list - see [SkillMulti]. Enabling `PauseButton` over [SkillMulti.SUB_RANGE] on
 * each of them is what makes the click reportable at all, and the range has to span the quantities
 * rather than the slots because the client puts the chosen count in the subcomponent field.
 */
internal fun Player.ifSkillMulti(
    ...
    // Not `ifOpenChat`: `chatbox:chatmodal` is 479x96, sized for dialogue boxes, while
    // `skillmulti:universe` is a fixed 519x142 that centres itself in its parent - so targeting the
    // chat modal overflowed it by 20px each side and 23px top and bottom, clipping the title. The
    // chat *area* is 519 by the chatbox height less its 23px tab strip, which is exactly 519x142.
...
internal fun Player.ifChoice(
    ...
    // The option list can be taller than the 479x96 dialogue modal. Mount it on the full chat
    // area so the title and the last option stay inside the parent in both fixed and resizable
    // layouts. ProtectedAccess uses the same target while waiting for the click.
    chatModalUnclamp = constants.modal_infinitewidthandheight
    topLevelChatboxResetBackground(this)
    openModal(interfaces.chatmenu, components.chatbox_chatarea, eventBus)
...
/**
 * Dialogue text is top-aligned inside the cache's safe-zone. Center alignment made short NPC and
 * message-box pages begin below the intended baseline and pushed the last line down on compact
 * fixed-mode chatboxes. Keeping this in one constant makes every dialogue family use the same
 * vertical origin.
 */
private const val DIALOGUE_TEXT_ALIGN_V: Int = 0
