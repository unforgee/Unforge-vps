...
public class PlayerUInteractions @Inject constructor(private val eventBus: EventBus) {
    ...
    public suspend fun interactOp(
        ...
        // Game-mode gate: using items on other players is a potential item-transfer path, so
        // restricted modes are denied before any script can fire.
        val useCheck = IronmanPolicy.canUseItemOn(access.player, target)
        if (!useCheck.allowed) {
            useCheck.denialMessage?.let { access.player.mes(it) }
            return
        }
        val obj = inv[invSlot]
