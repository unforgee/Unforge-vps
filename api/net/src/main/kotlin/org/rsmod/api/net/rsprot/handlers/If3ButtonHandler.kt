...
constructor(
    ...
    override fun handle(player: Player, message: If3Button) {
        val isBankComponent = message.interfaceId == 12 || message.interfaceId == 15
        if (isBankComponent) {
            logger.info {
                "[BANK-CLICK] if=${message.interfaceId} comp=${message.componentId} " +
                    "sub=${message.sub} op=${message.op} obj=${message.obj} player=${player.username}"
            }
        }
        ...
        if (!opEnabled) {
            logger.info { "[REJECTED] If3Button: $message player=${player.username}" }
            return
        }
        ...
        if (player.ui.containsModal(interfaceType)) {
            ...
            if (player.isModalButtonProtected) {
                logger.info { "[Modal][BLOCKED] If3Button: $message player=${player.username}" }
                return
            }
            protectedAccess.launchLenient(player) { eventBus.publish(this, event) }
        } else if (isBankComponent) {
            logger.info {
                "[BANK-CLICK][NO-ROUTE] if=${message.interfaceId} comp=${message.componentId} " +
                    "overlay=${player.ui.containsOverlay(interfaceType)} " +
                    "modal=${player.ui.containsModal(interfaceType)} " +
                    "toplevel=${player.ui.containsTopLevel(interfaceType)} player=${player.username}"
            }
        }
    }
