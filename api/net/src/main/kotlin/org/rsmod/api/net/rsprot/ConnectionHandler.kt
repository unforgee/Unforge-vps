...
private constructor(
    ...
    override fun onLogin(
        ...
        // The local dev realm serves an enriched JS5 cache. A fresh remote client can
        // legitimately arrive with the vanilla CRC set before it has completed the JS5
        // update round-trip, so do not reject that first login in dev mode. Production
        // realms must keep the strict cache-version gate.
        if (!block.crc.validate(js5Crc) && !realm.config.devMode) {
