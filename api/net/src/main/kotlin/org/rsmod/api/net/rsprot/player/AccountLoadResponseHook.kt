...
class AccountLoadResponseHook(
    ...
    private fun validateAndQueueLogin(response: AccountLoadResponse.Ok.LoadAccount) {
        ...
        //
        // Accounts that have not completed the first-login game mode selection are exempt: a
        // disconnect during selection must not lock the account out - the next login simply
        // reopens the selector. Nothing gameplay-related could have happened before selection,
        // so there is no save state to be "partial".
        val isPartialSave = response.account.lastLogout == null && response.account.gameModeSelected
        if (isPartialSave) {
            logger.error {
    ...
    private fun Player.applyConfigTransforms(config: RealmConfig) {
        // Realm-wide grants are re-applied on every login, not just the first one. An account that
        // was created before the realm enabled them - or whose first login never saved - would
        // otherwise stay nameless forever, which locks the chatbox (and with it `::` commands).
        if (config.autoAssignDisplayNames && displayName.isBlank()) {
            displayName = username.toDisplayName()
        }
        // Standalone realm / server: every player receives full access so that all items and
        // features are usable.
        members = true
        if (config.devMode) {
        ...
        // Placement and xp rate are first-login only: returning players keep where they logged out.
    ...
    private fun handleGameLogin(player: Player, loadResponse: AccountLoadResponse.Ok) {
        ...
        // Set the full-access value immediately before creating the login packet so stale
        // account/character data can never restrict one login.
        player.members = true
